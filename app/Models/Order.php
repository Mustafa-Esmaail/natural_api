...
class Order extends Model
{
    ...
    const STATUS_READY_TO_SHIPPING      = 'ready_to_shipping';
    ...
    const STATUSES = [
        ...
        self::STATUS_READY_TO_SHIPPING      => self::STATUS_READY_TO_SHIPPING,
    ...
    public function paymentRecicpt(): HasOne
    {
    ...
    public function scopeFilter($query, array $filter): void
    {
        ...
        $query
            ->when(isset($filter['id']),    fn ($q) => $q->whereIn('id', $filter['id']))
            ...
            ->when(data_get($filter, 'shipping_company_id'), function ($q, $shipping_company_id) {
                $q->whereHas('shippingOrder', fn ($q) => $q->where('company_id', $shipping_company_id));
            })
            ->when(data_get($filter, 'product_id'), function ($q, $product_id) {
                $q->whereHas('orderDetails', function ($q) use ($product_id) {
                    $q->whereHas('stock', function ($q) use ($product_id) {
            ...
            ->when(
                data_get($filter, 'deliveryman_id'),
                fn (Builder $q, $deliverymanId) =>
            ...
            ->when(data_get($filter, 'search'), function ($q, $search) {
                $q->where(function ($b) use ($search) {
                    $b
                        ...
                        ->orWhereHas('shippingOrder', fn ($q) => $q->where('awb', 'LIKE', "%$search%"))
