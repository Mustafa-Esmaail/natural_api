...
class Cart extends Model
{
    ...
    public function scopeFilter($query, array $filter) {
        ...
        $query
            ...
            ->when(data_get($filter, 'ids'), fn($q, $ids) => $q->whereIn('id', $ids))
            ...
            // ->when(data_get($filter, 'date_from'), fn($q) => $q->where('created_at', '>=', data_get($filter, 'date_from') . "00:00:00"))
            // ->when(data_get($filter, 'date_to'), fn($q) => $q->where('created_at', '<=', data_get($filter, 'date_to'). "23:59:59"))
            ->when(data_get($filter, 'date_from'), function ($q) use ($filter) {
                $dateFrom = data_get($filter, 'date_from') . " 00:00:00";
                $dateTo = data_get($filter, 'date_to') ? data_get($filter, 'date_to') . " 23:59:59" : $dateFrom; // handle the case when date_from and date_to are the same
                return $q->whereBetween('created_at', [$dateFrom, $dateTo]);
            })
            ->when(data_get($filter, 'product_id'), function ($q, $product_id) {
                $q->whereHas('userCarts.cartDetails.cartDetailProducts.stock', function ($q) use ($product_id) {
