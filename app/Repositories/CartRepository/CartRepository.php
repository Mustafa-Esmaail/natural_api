...
class CartRepository extends CoreRepository
{
    ...
    public function cartReport(array $filter)
    {

        $query = $this->model()
            ->filter($filter)
            ->with('user')
            ->whereHas('userCarts.cartDetails.cartDetailProducts')
            ->with($this->with());

        if (data_get($filter, 'export') === 'excel') {

            $name = 'cart-report-' . Str::random(8);

            Excel::store(new CartExport($query->get()), "export/$name.xlsx", 'public', \Maatwebsite\Excel\Excel::XLSX);
