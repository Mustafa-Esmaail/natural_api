...
class PaymentPayloadService extends CoreService
{
    ...
    public function prepareValidate($data): array
    {
        ...
        if ($payment->tag === Payment::TAG_PAY_PAL) {
            ...
            return ['status' => true];
        } else if ($payment->tag === Payment::TAG_UPAYMENTS) {

            $validator = $this->upayment($data);
            ...
            return ['status' => true];
        } else if ($payment->tag === Payment::TAG_NOON) {

            $validator = $this->noon($data);
     ...
     else if ($payment->tag === Payment::TAG_TAP) {

        $validator = $this->tap($data);
    ...
    public function upayment(array $data): \Illuminate\Contracts\Validation\Validator|\Illuminate\Validation\Validator
    {
        return Validator::make($data, [

            'payload.upayment_merchant_id'    => 'required|string',
            'payload.upayment_user_id'        => 'required|string',
            'payload.upayment_password'       => 'required|string',
            'payload.upayment_api_key'        => 'required|string',

            'payload.upayment_test_mode'      => 'required|boolean',
        ]);
    }
    public function noon(array $data): \Illuminate\Contracts\Validation\Validator|\Illuminate\Validation\Validator
    {
        return Validator::make($data, [
            'payload.noon_name'                      => 'required|string',
            'payload.noon_Auth_key'                  => 'required|string',
            'payload.noon_test_mode'                 => 'required|in:Test,Live',
            'payload.noon_support_marketplace'       => 'required|boolean',
        ]);
    }
    public function tap(array $data): \Illuminate\Contracts\Validation\Validator|\Illuminate\Validation\Validator
    {
        return Validator::make($data, [
            'payload.tap_public_key'                      => 'required|string',
            'payload.tap_private_key'                  => 'required|string',
