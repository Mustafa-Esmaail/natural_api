    ...
    public function create(array $data, array $notes = []): array
    {
        ...
        foreach ($cart->userCarts as $userCart) {
            ...
            foreach ($cartDetails as $cartDetail) {
                ...
                // dd($order->deliveryPoint->city->translation);


                // store paymentReceipt
                if (data_get($data, 'payment_id') == 15) {
                    $paymentReceipt = PaymentReceipt::create([
                        'img' => data_get($data, 'paymentReceipt'),
                ...
                if (data_get($data, 'shipping_company_id') && data_get($data, 'delivery_type') === 'delivery') {
                    $shippingOrder = ShippingOrder::create([
                        'status' => 'new',
                        'order_id' => $order->id,
                        'company_id' => data_get($data, 'shipping_company_id'),
                    ]);
                }
        ...
        if (in_array(data_get($data, 'payment_id'), [15, 1, 2])) {
            $cart->delete();
         }
