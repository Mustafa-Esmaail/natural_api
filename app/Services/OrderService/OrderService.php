...
class OrderService extends CoreService
{
    ...
    public function UpdateOrderStatus(?array $ids = [],  string $status): array
    {
        $errors = [];

        $orders = Order::find(is_array($ids) ? $ids : []);
    ...
    private function calculateOrder(Order $order, array $data, bool $isUpdate = false, int $ordersCount = 0): void
    {
        ...
        // if(data_get($data, 'delivery_type')==='point'){
        // $totalPrice += $deliveryFeeSum;

        // }
        // else {
        //     $shipingCompany = ShippingCompany::find(data_get($data, 'shipping_company_id'));
        //     if (data_get($data, 'payment_id') === 1) {
        //         $shippingFee = $shipingCompany->cash_on_delivery ? $shipingCompany->price + $shipingCompany->cash_fee : $shipingCompany->price;
        //     } else {
        //         $shippingFee = $shipingCompany->price;
        //     }
        // $totalPrice += $shippingFee;
        // $deliveryFeeSum = $shippingFee;
    ...
    private function createOrderCoupon(Coupon $coupon, Order $order, $totalPrice): float|int|null
    {
        ...
        if ($coupon->type === 'percent') {
            $couponPrice =  ($totalPrice / 100) * $coupon->price;
            if ($couponPrice > $coupon->max_discount_value) {
                $couponPrice = $coupon->max_discount_value;
            }
        } else {
        ...
        $orderCoupone = OrderCoupon::updateOrCreate([
            'user_id' => $order->user_id,
            'name'    => $coupon->name,
            'price'   => $couponPrice,
            'order_id'   => $order->id,
    ...
    public function updateStatus(?array $ids = [], array $array, ?int $shopId = null): array
    {
        $errors = [];
        ...
        foreach ($orders as $order) {
            ...
            try {
                DB::transaction(function () use ($order, $array) {


                    $order->update([
                        'status' => data_get($array, 'status')
                    ]);
                });
            } catch (Throwable $e) {
                $errors[] = $order->id;
