...
class TransactionController extends PaymentBaseController
{
    ...
    public function updateStatus(string $type,  $id, TransactionUpdateRequest $request): JsonResponse
    {
        /** @var Order $model */
        $ids=[];

        if($id=="tap_id"){
            $ids = Transaction::where('payment_trx_id', $request->input('payment_trx_id'))->pluck('payable_id')->toArray();
        }
        else{
            $ids = explode('-', $id); // Convert string to array using hyphen as the delimiter
        ...
        $model = match ($type) {
            ...
            default         => Order::with('transaction')->whereIn('id', $ids)->get(),
        };
        if ($type === 'order') {
