...
class OrderHelper
{
    ...
    public static function checkShopDelivery(
        ...
        if ($data['delivery_type'] === Order::DELIVERY && isset($data['delivery_price_id'])) {
            ...
            $deliveryPrice = self::deliveryPrice($shop, (int)$data['delivery_price_id'], $lang, (int)$data['shipping_company_id'], (int)$data['payment_id']);
     ...
     * @throws Exception
     */
    private static function deliveryPrice(Shop $shop, int $deliveryPriceId, string $lang, int $shipping_company_id, int $payment_id): array
    {
        ...
        //     }
        // }
        $shippingCompany = ShippingCompany::find($shipping_company_id);
        $shippingFee = 0;
        if ($shippingCompany) {
            if ($payment_id == 1) {

                $shippingFee = $shippingCompany->cash_on_delivery ? $shippingCompany->price + $shippingCompany->cash_fee : $shippingCompany->price;
                // $deliveryFee[]=$shippingFee;

            } else {
                $shippingFee = $shippingCompany->price;
                // $deliveryFee[]=$shippingFee;
            }
        }
    ...
    {
        try {
            $name = data_get($data, "coupon")[$shopId]['name'];
